...
> 💡 可以先用 Micro 起步，后续再升级。init 脚本加 `-Force` 参数会补充缺失文件，不会覆盖已有文件。
...
v2.0 物理凭证门禁（单卡开工契约、物理硬停机、铁三角门禁、防同谋自验）在所有模式中均启用。轻量模式保留了核心的「流程控制」和「防漂移」能力，只省略了「知识积累」相关文件。
