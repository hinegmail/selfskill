...
**Trigger**: `.ai/STATUS.md` or `.ai/NEXT.md` missing; or user says "启动项目 / init / setup".
**Actions**: Read existing `.ai/` docs → auto-extract STEERING.md if missing → propose initial files. **No code.** Wait for confirmation.
...
**Must update (BLOCKING)**: TASKS.md (mark [x]), STATUS.md (TL;DR + phase summary + **milestone status update** + timestamp), NEXT.md (set next active task; if none, write **exactly** `no active task` — no other idle-state text allowed).
...
2. STATUS.md TL;DR — mentions completed task + milestone status updated.
...
**Generated**: 2026-07-04T18:07:03.704421Z
